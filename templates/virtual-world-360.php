<?php
get_header('header');
/*
Template Name: virtual world
*/
?>
<!-- Hero Slider -->
<section id="aboutUsContainer" class="aboutUsContainer" style="background-image: url('<?php echo get_field('banner_image'); ?>');">
  <div class="heading-top">

  </div>
</section>
<div class="container">
  <div class="breadcrumb-link">
    <ul>
      <li><a href="">Home</a></li>
      <li>Virtual world</li>
    </ul>
  </div>
</div>
<section class="virtual-tour-heading section-padding virtual-tour-360">
  <div class="container">
    <div class="projects-list-section-inner-heading">
      <div class="projects-list-section-inner-heading-right">
        <h2 class="text-uppercase"><?php echo get_field('section_1_heading');?> <span> <?php echo get_field('section_1_color_heading');?> </span> <?php echo get_field('section_1_normal_heading');?></h2>
      </div>
      <p><?php echo get_field('section_1_subheading');?>
      </p>
    </div>
  </div>
</section>
<div class="virtual-tour-360-box-heading">
  <div class="container">
    <div class="virtual-tour-360-box-heading-inner">
      <div class="virtual-tour-360-box-heading-left">
        <img src="<?php echo get_field('section_1_image');?>" alt="">
      </div>
      <div class="virtual-tour-360-box-heading-right">
        <div class="select-menu">
          <div class="select-btn">
            <span class="sBtn-text">Select your menu</span>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
              <path d="M297.4 438.6C309.9 451.1 330.2 451.1 342.7 438.6L502.7 278.6C515.2 266.1 515.2 245.8 502.7 233.3C490.2 220.8 469.9 220.8 457.4 233.3L320 370.7L182.6 233.4C170.1 220.9 149.8 220.9 137.3 233.4C124.8 245.9 124.8 266.2 137.3 278.7L297.3 438.7z" />
            </svg>
          </div>
          <ul class="options">
            <li class="option">
              <span class="option-text">Start</span>
            </li>
            <li class="option">
              <span class="option-text">Floorplan</span>
            </li>
            <li class="option">
              <span class="option-text">Entrance</span>
            </li>
            <li class="option">
              <span class="option-text">Kitchen</span>
            </li>
            <li class="option">
              <span class="option-text">Dining 1</span>
            </li>
            <li class="option">
              <span class="option-text">Dining 2</span>
            </li>
            <li class="option">
              <span class="option-text">Living</span>
            </li>
            <li class="option">
              <span class="option-text">Hall 1</span>
            </li>
            <li class="option">
              <span class="option-text">Hall 2</span>
            </li>
            <li class="option">
              <span class="option-text">Bedroom</span>
            </li>
            <li class="option">
              <span class="option-text">Dressing Room</span>
            </li>
            <li class="option">
              <span class="option-text">Home Office 1</span>
            </li>
            <li class="option">
              <span class="option-text">Home Office 2</span>
            </li>
            <li class="option">
              <span class="option-text">Children's Room 1 </span>
            </li>
            <li class="option">
              <span class="option-text">Children's Room 2 </span>
            </li>
            <li class="option">
              <span class="option-text">Guest Room 1 </span>
            </li>
            <li class="option">
              <span class="option-text">Guest Room 2 </span>
            </li>
          </ul>
        </div>
        <div class="language-changing">
          <button>DE</button>
          <span class="language-changing-line"></span>
          <button>EN</button>
        </div>
      </div>
    </div>
  </div>
</div>
<div class="virtual-tour-360-box">
  <div class="container">
    <div id="virtual_tour_360"></div>
  </div>
</div>
<div class="download_section_virtual light_grey">
  <div class="container">
    <div class="download_pdf_awards">
      <a href="" class="btn_1">
        <span class="btn-inner">
          <span class="svg-span"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
              <path d="M352 96C352 78.3 337.7 64 320 64C302.3 64 288 78.3 288 96L288 306.7L246.6 265.3C234.1 252.8 213.8 252.8 201.3 265.3C188.8 277.8 188.8 298.1 201.3 310.6L297.3 406.6C309.8 419.1 330.1 419.1 342.6 406.6L438.6 310.6C451.1 298.1 451.1 277.8 438.6 265.3C426.1 252.8 405.8 252.8 393.3 265.3L352 306.7L352 96zM160 384C124.7 384 96 412.7 96 448L96 480C96 515.3 124.7 544 160 544L480 544C515.3 544 544 515.3 544 480L544 448C544 412.7 515.3 384 480 384L433.1 384L376.5 440.6C345.3 471.8 294.6 471.8 263.4 440.6L206.9 384L160 384zM464 440C477.3 440 488 450.7 488 464C488 477.3 477.3 488 464 488C450.7 488 440 477.3 440 464C440 450.7 450.7 440 464 440z"></path>
            </svg></span>
          <span class="text-span">Exhibit overview (PDF)</span>
        </span>
      </a>
    </div>
  </div>
</div>
<section class="section-padding form-section-bottom-side">
  <div class="container">
    <div class="form-section-bottom-side-inner">
      <div class="form-section-bottom-left">
        <div class="form-section-subtitle">
          <span> <?php echo get_field('section_2_title');?></span>
        </div>
        <div class="form-section-h2">
          <h2><?php echo get_field('section_2_heading');?> <span><?php echo get_field('section_2_color_heading');?> </span> <?php echo get_field('section_2_normal_heading');?> </h2>
        </div>
        <div class="form-section-p">
          <p><?php echo get_field('section_2_subheading');?></p>
        </div>
      </div>
      <div class="form-section-bottom-right">
        <div class="contact-info-inner-bottom-right-form">
          <div class="contact-info-inner-bottom-right-1">
            <div class="contact-bottom-right-1-left">
              <label for>Full Name *</label>
              <input type="text"
                placeholder="Your Name">
            </div>
            <div class="contact-bottom-right-2-right">
              <label for>Phone *</label>
              <input type="text" placeholder="Phone">
            </div>
          </div>
          <div class="contact-info-inner-bottom-right-2">
            <div class="contact-bottom-right-2-left">
              <label for>Email Address *</label>
              <input type="text"
                placeholder="Email Address">
            </div>
            <div class="contact-bottom-right-2-right">
              <label for>Location *</label>
              <input type="text"
                placeholder="Location">
            </div>
          </div>
          <div class="contact-info-inner-bottom-right-3">
            <div class="right-3-textarea">
              <label for>Your Message</label>
              <textarea cols="40" rows="4" name id
                placeholder="Your Message"></textarea>
            </div>
          </div>
        </div>
        <div class="contact-info-btn">
          <a href class="btn_1">
            <span class="btn-inner">
              <span class="text-span">Send
                Message</span>
              <span class="svg-span"><svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 640 640">
                  <path
                    d="M566.6 342.6C579.1 330.1 579.1 309.8 566.6 297.3L406.6 137.3C394.1 124.8 373.8 124.8 361.3 137.3C348.8 149.8 348.8 170.1 361.3 182.6L466.7 288L96 288C78.3 288 64 302.3 64 320C64 337.7 78.3 352 96 352L466.7 352L361.3 457.4C348.8 469.9 348.8 490.2 361.3 502.7C373.8 515.2 394.1 515.2 406.6 502.7L566.6 342.7z" />
                </svg></span>
            </span>
          </a>
        </div>
      </div>
    </div>
  </div>
</section>